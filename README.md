...
## Автосборка результатов

Если нужно одной командой собрать все материалы из `student-project/` в `results/`, проект поддерживает автоматическое правило:

- если у `.md` есть frontmatter с `marp: true`, файл конвертируется в `.pptx`;
- все остальные `.md` конвертируются в `.pdf`;
- структура директорий сохраняется относительно `student-project/`.
